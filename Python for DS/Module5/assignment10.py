...
path = r'F:\Juhon\python\EdX Azure\Python for DS\Module5\Datasets\recordings'
...
for file in os.listdir(path):
    if file.startswith('0'):
        audio = wavfile.read(os.path.join(path, file))
        sample_rate = audio[0]
        zero.append(audio[1])
...
zero = pd.DataFrame(zero, dtype='int16')
...
cut = int(Provided_Portion*n_audio_samples)
X_test = test[:cut]
